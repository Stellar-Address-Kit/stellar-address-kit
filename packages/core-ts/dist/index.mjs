...
function decodeMuxed(mAddress) {
  ...
  return {
    baseG: StrKey2.encodeEd25519PublicKey(Buffer.from(pubkey)),
    id
  };
}
...
// src/muxed/encode.ts
import { MuxedAccount, StrKey as StrKey3 } from "@stellar/stellar-sdk";
...
function encodeMuxed(baseG, id) {
  ...
  if (!StrKey3.isValidEd25519PublicKey(baseG)) {
...
function extractRouting(input) {
  ...
  if (parsed.kind === "C") {
    ...
    warnings2.push({
      ...
      severity: "warn",
      message: "Contract address detected. Contract addresses cannot be used as transaction senders."
    });
    return {
      destinationBaseAccount: null,
      routingId: null,
